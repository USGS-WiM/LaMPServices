...
namespace LaMPServices.Authentication
{
    public class LaMPBasicAuthentication : IBasicAuthenticator
    {
        ...
        public AuthenticationResult Authenticate(BasicAuthRequestHeader header)
        {
            try
            {
                using (LaMPDSEntities aLDSE = new LaMPDSEntities(string.Format(@"metadata=res://*/LaMPDSModel.csdl|res://*/LaMPDSModel.ssdl|res://*/LaMPDSModel.msl;provider=Oracle.DataAccess.Client;provider connection string=""DATA SOURCE=LAMPRDS;USER ID={0};PASSWORD={1}""", header.Username, header.Password)))
                {
