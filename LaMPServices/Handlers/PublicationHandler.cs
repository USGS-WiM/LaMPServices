...
namespace LaMPServices.Handlers
{
    public class PublicationHandler:HandlerBase
    {
        ...
        public OperationResult RemoveProjectPublication(Int32 projectId, Int32 publicationId)// PUBLICATION aPublication)
        {
            //Return BadRequest if missing required fields
            if (projectId <= 0 || publicationId <= 0)
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    //first check if Project already contains publication
                    PROJECT_PUBLICATIONS thisProjPub = aLaMPRDS.PROJECT_PUBLICATIONS.FirstOrDefault(pc => pc.PUBLICATION_ID == publicationId &&
