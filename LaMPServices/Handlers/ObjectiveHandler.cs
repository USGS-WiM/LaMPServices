...
namespace LaMPServices.Handlers
{
    public class ObjectiveHandler : HandlerBase
    {
        ...
        public OperationResult Get()
        {
            List<OBJECTIVE_TYPE> objList;
                ...
                using (LaMPDSEntities aLaMPRDS = GetRDS())
                {
                    ...
                    objList = aLaMPRDS.OBJECTIVE_TYPE.OrderBy(o => o.OBJECTIVE).ToList();
        ...
        public OperationResult AddProjectObjective(Int32 projectId, OBJECTIVE_TYPE aObjective)
        {
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    //add to project
                    //first check if Project already contains objective
        ...
        public OperationResult RemoveProjectObjective(Int32 projectId, Int32 objectiveId)
        {
            //Return BadRequest if missing required fields
            if (projectId <= 0 || objectiveId <= 0)
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    //remove from project
                    //first check if Project already contains objective
                    PROJECT_OBJECTIVES thisProjObjective = aLaMPRDS.PROJECT_OBJECTIVES.FirstOrDefault(po => po.OBJECTIVE_ID == objectiveId &&
                                                                         po.PROJECT_ID == projectId);
                    
                    if (thisProjObjective != null)
                    {//remove it
                        aLaMPRDS.PROJECT_OBJECTIVES.DeleteObject(thisProjObjective);
                        aLaMPRDS.SaveChanges();
                    }//end if
        ...
        private Boolean Exists(System.Data.Objects.IObjectSet<OBJECTIVE_TYPE> objectiveTypes, ref OBJECTIVE_TYPE aObjective)
        {
            ...
            try
            {
                existingOBJ = objectiveTypes.FirstOrDefault(o => o.OBJECTIVE.ToUpper().Equals(thisOBJ.OBJECTIVE.ToUpper()));
