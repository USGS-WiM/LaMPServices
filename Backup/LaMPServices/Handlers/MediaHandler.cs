...
namespace LaMPServices.Handlers
{
    public class MediaHandler: HandlerBase
    {
        ...
        public OperationResult Delete(Int32 mediaTypeId)
        {
            //Return BadRequest if missing required fields
            if (mediaTypeId <= 0)
            { return new OperationResult.BadRequest();}
        ...
        public OperationResult RemoveSiteMedium(Int32 siteId, MEDIA_TYPE aMedia)
        {
            //Return BadRequest if missing required fields
            if (siteId <= 0 || String.IsNullOrEmpty(aMedia.MEDIA))
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    SITE_MEDIA thisCatMed = aLaMPRDS.SITE_MEDIA.FirstOrDefault(cm => cm.MEDIA_TYPE_ID == aMedia.MEDIA_TYPE_ID &&
                                                                         cm.SITE_ID == siteId);
                    if (thisCatMed != null)
                    {//remove it
