...
namespace LaMPServices.Handlers
{
    public class ContactHandler: HandlerBase
    {
        ...
        public OperationResult RemoveProjectContact(Int32 projectId, CONTACT aContact)
        {
            //Return BadRequest if missing required fields
            if (projectId <= 0 || String.IsNullOrEmpty(aContact.NAME))
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    PROJECT_CONTACTS thisProjContact = aLaMPRDS.PROJECT_CONTACTS.FirstOrDefault(pc => pc.CONTACT_ID == aContact.CONTACT_ID &&
