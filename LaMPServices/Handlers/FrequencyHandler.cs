...
namespace LaMPServices.Handlers
{
    public class FrequencyHandler:HandlerBase
    {
        ...
        public OperationResult RemoveSiteFrequency(Int32 siteId, Int32 frequencyId)// FREQUENCY_TYPE aFrequencyType)
        {
            //Return BadRequest if missing required fields
            if (siteId <= 0 || frequencyId <= 0)
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    SITE_FREQUENCY thisSiteFreq = aLaMPRDS.SITE_FREQUENCY.FirstOrDefault(cf => cf.FREQUENCY_TYPE_ID == frequencyId &&
                                                                        cf.SITE_ID == siteId);
