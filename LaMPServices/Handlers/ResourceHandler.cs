...
namespace LaMPServices.Handlers
{
    public class ResourceHandler:HandlerBase
    {
        ...
        public OperationResult RemoveSiteResourceType(Int32 siteId, Int32 resourceTypeId)// RESOURCE_TYPE aResourceType)
        {
            //Return BadRequest if missing required fields
            if (siteId <= 0 || resourceTypeId <= 0)
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    SITE_RESOURCE thisCatRes = aLaMPRDS.SITE_RESOURCE.FirstOrDefault(cr => cr.RESOURCE_TYPE_ID == resourceTypeId &&
                                                                         cr.SITE_ID == siteId);
                    if (thisCatRes != null)
                    {//remove it
