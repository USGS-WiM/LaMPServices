...
namespace LaMPWeb.Controllers
{
    ...
    {  
        ...
        public ActionResult ProjectCreate()
        {
            ...
            ViewData["ProjDurations"] = GetAllProjDurations();
            ViewData["ProjStatuses"] = GetAllProjStats();
        ...
        public ActionResult ProjectCreate(ProjectModel pm, string Create)
        {
            try
            {
                ...
                //update project and site counts
                request = new RestRequest();
                request.Resource = "/dataManagers/{dataManagerId}/projects";
                request.RootElement = "ArrayOfPRODUCT";
                request.AddParameter("dataManagerId", postedProject.DATA_MANAGER_ID, ParameterType.UrlSegment);
                List<PROJECT> ProjList = serviceCaller.Execute<List<PROJECT>>(request);

                if (ProjList.Count > 0)
                {
                    Session["TotProj"] = String.Format("{0:n0}", ProjList.Count());
                }
        ...
        public ActionResult ProjectDetails(int id)
        {
            ...
            //parse out urls 
            if (thisProject.URL != null)
            {
                if (thisProject.URL.Contains("|"))
                {
                    ViewData["eachURLarray"] = Regex.Split(thisProject.URL, "|");
                }
            }
            ...
            ViewData["projDuration"] = GetProjDuration(id);
            ViewData["projStatus"] = GetProjStatus(id);
        ...
        public ActionResult ProjectEdit(int id)
        {
            ...
            PROJECT thisProject = GetThisProject(id);
            if (!string.IsNullOrEmpty(thisProject.URL))
            {
                if (thisProject.URL.Contains("|"))
                {
                    //multiple urls
                ...
                {
                    //only 1 url
                    string[] oneUrlArray = new string[1];
                    oneUrlArray[0] = thisProject.URL;
                    ViewData["eachURLarray"] = oneUrlArray;
                }
            }
        ...
        public ActionResult ProjectDelete(int id)
        {
            try
            {
                ...
                //get the dm
                var request = new RestRequest();
                request.Resource = "/projects/{projectId}/dataManager";
                request.RootElement = "DATA_MANAGER";
                request.AddParameter("projectId", id, ParameterType.UrlSegment);
                ...
                request.Resource = "/dataManagers/{dataManagerId}/projects";
                request.RootElement = "ArrayOfPRODUCT";
                request.AddParameter("dataManagerId", thisDM.DATA_MANAGER_ID, ParameterType.UrlSegment);
                List<PROJECT> ProjList = serviceCaller.Execute<List<PROJECT>>(request);

                if (ProjList.Count > 0)
                {
                    Session["TotProj"] = String.Format("{0:n0}", ProjList.Count());
                    //Session["TotSites"] = String.Format("{0:n0}", ProjSiteCts.SitesCount);
                }
        ...
        [HttpPost]
        public JsonResult ProjectFlag(int flag, int projectId)
        {
            PROJECT thisProject = GetThisProject(projectId);
            
            LaMPServiceCaller serviceCaller = LaMPServiceCaller.Instance;
            var request = new RestRequest(Method.POST);

            request.Resource = "/projects/{projectId}?Flag={flag}";
            request.RequestFormat = DataFormat.Xml;
            request.AddParameter("flag", flag, ParameterType.UrlSegment);
            request.AddParameter("projectId", projectId, ParameterType.UrlSegment);
            request.AddHeader("X-HTTP-Method-Override", "PUT");
            request.AddHeader("Content-Type", "application/xml");
            request.XmlSerializer = new RestSharp.Serializers.DotNetXmlSerializer();
            request.AddBody(thisProject);
            PROJECT updatedProj = serviceCaller.Execute<PROJECT>(request);
        ...
        private object GetProjStatus(int id)
        {
            string status_value = string.Empty;

            LaMPServiceCaller serviceCaller = LaMPServiceCaller.Instance;
            var request = new RestRequest();
            request.Resource = "/Projects/{projectId}/projStatus";
            request.RootElement = "PROJ_STATUS";
            request.AddParameter("projectId", id, ParameterType.UrlSegment);
            PROJ_STATUS ProjStat = serviceCaller.Execute<PROJ_STATUS>(request);
            status_value = ProjStat != null ? ProjStat.STATUS_VALUE : "";

            return status_value;
        }

        private object GetProjDuration(int id)
        {
            string duration_value = string.Empty;

            LaMPServiceCaller serviceCaller = LaMPServiceCaller.Instance;
            var request = new RestRequest();
            request.Resource = "/Projects/{projectId}/projDuration";
            request.RootElement = "PROJ_DURATION";
            request.AddParameter("projectId", id, ParameterType.UrlSegment);
            PROJ_DURATION ProjDuration = serviceCaller.Execute<PROJ_DURATION>(request);
            duration_value = ProjDuration != null ? ProjDuration.DURATION_VALUE : "";
        ...
        private object GetAllProjStats()
        {
            LaMPServiceCaller serviceCaller = LaMPServiceCaller.Instance;
            var request = new RestRequest();
            request.Resource = "/ProjectStatus";
            request.RootElement = "ArrayOfPROJ_STATUS";
            List<PROJ_STATUS> allStats = serviceCaller.Execute<List<PROJ_STATUS>>(request);
            return allStats;
        }

        private object GetAllProjDurations()
        {
            LaMPServiceCaller serviceCaller = LaMPServiceCaller.Instance;
            var request = new RestRequest();
            request.Resource = "/ProjectDuration";
            request.RootElement = "ArrayOfPROJ_DURATION";
            List<PROJ_DURATION> allDurations = serviceCaller.Execute<List<PROJ_DURATION>>(request);
            return allDurations;
        }
    }
}
