...
namespace LaMPServices.Handlers
{
    public class ParameterHandler:HandlerBase
    {
        ...
        public OperationResult ParametersByGroupName(string groupNames)
        {
            ...
            try
            {
                ...
                {                    
                    //return all of them
                    query = aLaMPRDS.PARAMETER_TYPE;
                    parameterGroups = new ParameterGroups();
                                         
                    //by groups (only if they passed this group)
                    if (groups.Contains("Physical"))
                    {
                        parameterGroups.Physical = query.Where(a => a.PARAMETER_GROUP.Equals("Physical", StringComparison.OrdinalIgnoreCase)).ToList();
                    }
                    if (groups.Contains("Chemical"))
                    {
                        parameterGroups.Chemical = query.Where(a => a.PARAMETER_GROUP.Equals("Chemical", StringComparison.OrdinalIgnoreCase)).ToList();
                    }
                    if (groups.Contains("Biological"))
                    {
                        parameterGroups.Biological = query.Where(a => a.PARAMETER_GROUP.Equals("Biological", StringComparison.OrdinalIgnoreCase)).ToList();
                    }
                    if (groups.Contains("Microbiological"))
                    {
                        parameterGroups.Microbiological = query.Where(a => a.PARAMETER_GROUP.Equals("Microbiological", StringComparison.OrdinalIgnoreCase)).ToList();
                    }
                    if (groups.Contains("Toxicological"))
                    {
                        parameterGroups.Toxicological = query.Where(a => a.PARAMETER_GROUP.Equals("Toxicological", StringComparison.OrdinalIgnoreCase)).ToList();
                    }

                }//end using

                return new OperationResult.OK { ResponseResource = parameterGroups };
            }//end try
            catch (Exception e)
            {
        ...
        [HttpOperation(HttpMethod.GET, ForUriName = "GetParameterGroups")]
        public OperationResult GetParameterGroups()
        {
            List<PARAMETER_TYPE> parameters;

            using (LaMPDSEntities aLaMPRDS = GetRDS())
            {
                List<PARAMETER_TYPE> query = aLaMPRDS.PARAMETER_TYPE.ToList();

                parameters = query.GroupBy(p => p.PARAMETER_GROUP).Select(g => g.FirstOrDefault()).ToList();
                parameters = parameters.OrderBy(x => x.PARAMETER_GROUP).ToList();
        ...
        public OperationResult RemoveSiteParameter(Int32 siteId, Int32 parameterId)// PARAMETER_TYPE aParameter)
        {
            //Return BadRequest if missing required fields
            if (siteId <= 0 || parameterId <= 0)
            { return new OperationResult.BadRequest(); }
            ...
            using (EasySecureString securedPassword = GetSecuredPassword())
            {
                using (LaMPDSEntities aLaMPRDS = GetRDS(securedPassword))
                {
                    ...
                    //remove from site
                    SITE_PARAMETERS thisSiteParameter = aLaMPRDS.SITE_PARAMETERS.FirstOrDefault(cp => cp.PARAMETER_TYPE_ID == parameterId &&
                                                                         cp.SITE_ID == siteId);
